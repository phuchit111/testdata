...
def run() -> dict:
    ...
    expected_methods = {"naive_last_day", "moving_avg_4w", "exp_smoothing_4w", "seasonal_naive_7d", "weekday_median_8w", "level_weekday_blend"}
    check("backtest benchmarks six methods", set(backtest["method"]) == expected_methods and forecast_metrics["selected_method"] == "level_weekday_blend", sorted(expected_methods), {"methods": sorted(backtest["method"].unique().tolist()), "selected": forecast_metrics["selected_method"]}, "A5")
    ...
    waste_rate_error = float((implied_waste_rate - inventory_policy["waste_rate"]).abs().max())
    check("Kitchen x SKU prep reconciles to displayed waste rate", waste_rate_error < 1e-10, "max error < 1e-10", waste_rate_error, "A5")
